...
class Test_Plugin_Base extends \WP_UnitTestCase {
	...
	public function test_locate_plugin() {
		...
		$this->assertEquals( 'tackle', $location['dir_basename'] );
		$this->assertContains( 'plugins/tackle', $location['dir_path'] );
		$this->assertContains( 'plugins/tackle', $location['dir_url'] );
	}
	...
	public function test_relative_path() {
		$this->assertEquals( 'plugins/tackle', $this->plugin->relative_path( '/srv/www/wordpress-develop/src/wp-content/plugins/tackle', 'wp-content', '/' ) );
		$this->assertEquals( 'themes/twentysixteen/plugins/tackle', $this->plugin->relative_path( '/srv/www/wordpress-develop/src/wp-content/themes/twentysixteen/plugins/tackle', 'wp-content', '/' ) );
	}
	...
	public function test_trigger_warning() {
		...
		set_error_handler( function ( $errno, $errstr ) use ( $obj ) {
			$obj->assertEquals( 'Tackle\Plugin: Param is 0!', $errstr );
	...
	public function test_add_doc_hooks_error() {
		$mock = $this->getMockBuilder( 'Tackle\Plugin' )
